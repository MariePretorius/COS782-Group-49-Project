"""Create calibrated Monolingual English RoBERTa outputs from an existing predictions.csv.
...
from run_enhanced_english_roberta import (
    PAIRWISE_OUTPUT_DIR,
...
def load_existing_predictions() -> pd.DataFrame:
    """Load strict Monolingual English RoBERTa predictions produced by run_enhanced_english_roberta.py."""
    if not PREDICTIONS_PATH.exists():
        raise FileNotFoundError(
            f"Missing Monolingual English RoBERTa predictions: {PREDICTIONS_PATH.resolve()}. "
            "Run src/run_enhanced_english_roberta.py first."
        )
...
def write_prior_calibrated(predictions: pd.DataFrame, calibration_df: pd.DataFrame) -> None:
    ...
    save_json(
        PRIOR_CALIBRATED_OUTPUT_DIR / "best_config.json",
        {
            ...
            "calibration_source": "existing_english_roberta_predictions_plus_train_validation_label_priors",
        },
    )
...
def write_pairwise(predictions: pd.DataFrame) -> None:
    ...
    direction = os.environ.get("COS760_ENGLISH_ROBERTA_PAIRWISE_DIRECTION", "lower")
...
def main() -> int:
    """Create post-processed Monolingual English RoBERTa outputs without retraining."""
    try:
        predictions = load_existing_predictions()
        calibration_df = load_calibration_data()
        calibration_df = calibration_df.loc[calibration_df["language"] == "eng"].copy()
