"""Enhanced Monolingual English RoBERTa experiment with validation selection and threshold tuning."""
...
OUTPUT_DIR = Path("outputs/enhanced/english_roberta")
PRIOR_CALIBRATED_OUTPUT_DIR = Path("outputs/enhanced/english_roberta_prior_calibrated")
PAIRWISE_OUTPUT_DIR = Path("outputs/enhanced/english_roberta_pairwise_exploratory")
COMPARISON_OUTPUT_DIR = Path("outputs/comparison")

MODEL_NAME = os.environ.get("COS760_ENGLISH_ROBERTA_MODEL", "FacebookAI/roberta-base")
MAX_LENGTH = int(os.environ.get("COS760_ENGLISH_ROBERTA_MAX_LENGTH", "128"))
TRAIN_BATCH_SIZE = int(os.environ.get("COS760_ENGLISH_ROBERTA_TRAIN_BATCH_SIZE", "8"))
EVAL_BATCH_SIZE = int(os.environ.get("COS760_ENGLISH_ROBERTA_EVAL_BATCH_SIZE", "8"))
NUM_TRAIN_EPOCHS = float(os.environ.get("COS760_ENGLISH_ROBERTA_NUM_TRAIN_EPOCHS", "2"))
WEIGHT_DECAY = 0.01
WARMUP_RATIO = 0.1
EARLY_STOPPING_PATIENCE = int(os.environ.get("COS760_ENGLISH_ROBERTA_EARLY_STOPPING_PATIENCE", "1"))
LEARNING_RATES = [
    float(value)
    for value in os.environ.get("COS760_ENGLISH_ROBERTA_LEARNING_RATES", "3e-5").split(",")
    if value.strip()
]
...
def load_datasets() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    train_df = train_df.loc[train_df["language"] == "eng"].copy()
    validation_df = validation_df.loc[validation_df["language"] == "eng"].copy()
    test_df = test_df.loc[test_df["language"] == "eng"].copy()
    validate_english_split_sets(
        {"train": train_df, "validation": validation_df, "test": test_df}
    )
    return train_df, validation_df, test_df


def validate_english_split_sets(split_frames: dict[str, pd.DataFrame]) -> None:
    """Validate monolingual English splits without expecting other languages."""
    for split, dataframe in split_frames.items():
        if dataframe.empty:
            raise ValueError(f"{split} set is empty after filtering to English.")

        languages = set(dataframe["language"].astype(str).unique())
        if languages != {"eng"}:
            raise ValueError(
                f"{split} set should contain only English rows, found: {sorted(languages)}"
            )
...
def pairwise_exploratory_predictions(
    ...
    The direction can be selected with COS760_ENGLISH_ROBERTA_PAIRWISE_DIRECTION=lower|higher.
    """
...
def update_comparison_outputs(
    ...
    """Add/replace an Monolingual English RoBERTa row in enhanced comparison outputs."""
...
def main() -> int:
    """Run enhanced Monolingual English RoBERTa with LR sweep and validation threshold tuning."""
    try:
        import torch
        ...
        for learning_rate in LEARNING_RATES:
            print(f"\nTraining Monolingual English RoBERTa with learning_rate={learning_rate}")
        ...
        calibrated_metrics, calibrated_per_language_metrics = evaluate_predictions(
            ...
            "english_roberta_prior_calibrated",
            include_probabilities=True,
        )
        save_json(
        ...
        pairwise_direction = os.environ.get("COS760_ENGLISH_ROBERTA_PAIRWISE_DIRECTION", "lower")
        ...
        pairwise_metrics, pairwise_per_language_metrics = evaluate_predictions(
            ...
            PAIRWISE_OUTPUT_DIR,
            "english_roberta_pairwise_exploratory",
            include_probabilities=True,
        )
        save_json(
