"""Create calibrated AfroXLMR outputs from an existing predictions.csv.
...
HYBRID_OUTPUT_DIR = Path("outputs/enhanced/hybrid_language_exploratory")
HYBRID_SOURCES = {
    "afr": Path("outputs/enhanced/afroxlmr_pairwise_exploratory/predictions.csv"),
    "eng": Path("outputs/enhanced/tfidf_char_logreg_prior_calibrated/predictions.csv"),
    "zul": Path("outputs/enhanced/afroxlmr_pairwise_exploratory/predictions.csv"),
}


def load_existing_predictions() -> pd.DataFrame:
    """Load strict AfroXLMR predictions produced by run_enhanced_afroxlmr.py."""
    if not PREDICTIONS_PATH.exists():
        raise FileNotFoundError(
            f"Missing AfroXLMR predictions: {PREDICTIONS_PATH.resolve()}. "
...
def write_prior_calibrated(predictions: pd.DataFrame, calibration_df: pd.DataFrame) -> None:
    ...
    save_json(
        PRIOR_CALIBRATED_OUTPUT_DIR / "best_config.json",
        {
            ...
            "calibration_source": "existing_afroxlmr_predictions_plus_train_validation_label_priors",
        },
    )
...
def main() -> int:
    """Create post-processed AfroXLMR outputs without retraining."""
    try:
